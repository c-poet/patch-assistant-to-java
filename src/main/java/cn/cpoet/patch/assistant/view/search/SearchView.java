...
import cn.cpoet.patch.assistant.core.SearchHisItem;
...
public class SearchView {
    ...
    private void searchHistory(Set<SearchItem> results) {
        ...
        List<SearchHisItem> history = search.getHistory();
        if (history != null) {
        ...
        if (index < keywords.length) {
            if (matchKeyword(keywords[index], node.getName())) {
                ...
                if (TreeNodeUtil.isCompressNode(node)) {
                    ...
                    List<TreeNode> oldChildren = node.getChildren();
                ...
                if (e.getButton() == MouseButton.PRIMARY) {
                    ...
                    if (e.getClickCount() == 2) {
                        if (item instanceof SearchNodeItem) {
                            ...
                            UIUtil.runNotUI(() -> {
                                SearchHisItem historyItem = new SearchHisItem();
                                historyItem.setName(view.searchField.getText());
                                historyItem.setPath("Search History");
                                historyItem.setTarget(node.getPath());
                                historyItem.setSearchTime(new Date());
